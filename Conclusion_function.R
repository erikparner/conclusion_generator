...
measure_to_percent <- function(number, measure="RR", decimals=2, interpret_measure="", interpret_direction=TRUE) {
...
conclusionBC <- function(estimate=NA,
  ...
  conclusion <- paste0("Assuming no uncontrolled biases, the point estimate of ",
                       ...
                       " corresponded to a ",
                       measure_to_percent(estimate, measure=measure, interpret_measure=paste0(measure_type, " ")),
  ...
  conclusion <- paste0(conclusion, 
                       ...
                       measure_to_percent(ci_lower, measure=measure),
                       " to a ",
                       measure_to_percent(ci_upper, measure=measure),
                       " in ",
                       measure_type, 
                       " from treatment. "
  )
  ...
  if(!is.na(superiority) & !is.na(noninferiority)) {
    ...
    mention_superiority_noninferiority <-  paste0("Given that a reduction of ",
                                                  measure_to_percent(superiority, measure=measure, interpret_direction=FALSE),
                                                  " or more would be considered clinically worth the cost and side effects of the treatment (if any)",
                                                  ", and an increase of up to ",
                                                  measure_to_percent(noninferiority, measure=measure, interpret_direction=FALSE),
